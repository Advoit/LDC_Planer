...
import {
  ...
  buildReportSlide,
  getInstandsetzungsreportTemplateBytes,
  instandsetzungsreportFileName,
  reportBeforeImages,
  reportAfterImages,
} from './instandsetzungsreport';
import type { InstandsetzungsreportCover } from './instandsetzungsreport';
import { photoGrid, VORHER_TILE, NACHHER_TILE } from './pptx-photo-tiles';
...
describe('buildReportSlide (mehrzeiliger Text, PowerPoint-kompatibel)', () => {
  ...
  it('setzt die Aufgabe-Beschreibung hinter „Hinweis zur Behebung:“ und den Hinweistext unten rechts', () => {
    ...
    const task = makeTask('T1', {
      ...
      fehlerbeschreibung: 'Haarrisse an der Fassade',
      hintText: 'Mit Fassadenfarbe überstreichen',
    });
    const xml = buildReportSlide(baseSlide, task, COVER, [], []);

    /* Beschreibung steht hinter „Hinweis zur Behebung:“ (Klammern entfernt) */
    expect(xml).toContain('Hinweis zur Behebung:');
    expect(xml).toContain('<a:t>Riss im Putz</a:t>');
    expect(xml).not.toContain('\u200B [');
    /* Hinweistext füllt das Textfeld unten rechts (statt Name + Beschreibung) */
    expect(xml).toContain('<a:t>Mit Fassadenfarbe überstreichen</a:t>');
    /* Aufgaben-Name steht nicht mehr im unteren Textfeld */
    expect(xml).not.toContain('<a:t>Aufgabe T1</a:t>');
    expectWellFormed(xml);
  });
});

describe('buildInstandsetzungsreportPptx', () => {
...
describe('instandsetzungsreportFileName', () => {
  it('bereinigt den Projektnamen', () => {
    const project = makeProject([]);
    project.name = 'Sanierung / Altbau!';
    expect(instandsetzungsreportFileName(project)).toBe(
      'Instandsetzungsreport-Sanierung-Altbau.pptx',
    );
  });
});
