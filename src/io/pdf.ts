...
import { PDFDocument, PDFFont, PDFImage, PDFName, PDFPage, StandardFonts, rgb } from 'pdf-lib';
...
/**
 * Legt eine unsichtbare Klickfläche (Link-Annotation) auf eine Seite, die zu
 * einer anderen Seite des Dokuments springt (z. B. für das Inhaltsverzeichnis).
 */
export function addInternalLink(
  ...
  const annotRef = context.register(
    context.obj({
      ...
      Border: [0, 0, 0],
      /* XYZ: Zielseite, Position oben links, Zoom unverändert (0) */
      Dest: [targetPage.ref, 'XYZ', 0, PAGE_HEIGHT, opts.zoom ?? 0],
    }),
  );
